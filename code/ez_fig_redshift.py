...
strm_rd = sdss_rd[(sdss_rd['z_phot0'] >= low) & (strm_rd['z_phot0'] <= high)]
low, high = np.percentile(llr_rd['z_phot'], [0.5, 99.5])
llr_rd = sdss_rd[(sdss_rd['z_phot'] >= low) & (llr_rd['z_phot'] <= high)]
low, high = np.percentile(xgb_rd['z_phot_xgb_origin'], [0.5, 99.5])
xgb_rd = sdss_rd[(sdss_rd['z_phot_xgb_origin'] >= low) & (xgb_rd['z_phot_xgb_origin'] <= high)]
low, high = np.percentile(sdss_rd['zsp'], [0.5, 99.5])
sdss_rd = sdss_rd[(sdss_rd['zsp'] >= low) & (sdss_rd['zsp'] <= high)]
